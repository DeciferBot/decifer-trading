...
## 19. Sprint 7J.1 — Two-Key Activation Gate Terms

These terms are added in Sprint 7J.1 to define the two-key activation model and publisher mode system.

| Term | Definition |
|------|-----------|
| **Two-Key Activation Gate** | The activation model implemented in Sprint 7J.1 requiring BOTH keys to be set for the live bot to consume the handoff universe: Key 1 = `enable_active_opportunity_universe_handoff = True` in `config.py` (bot config flag, set by Amit); Key 2 = publisher running in `controlled_activation` mode (`handoff_enabled = true` written to manifest, set by running `python3 handoff_publisher.py --mode controlled_activation`). Either key alone is insufficient. The live bot checks Key 1 (config flag gate) then calls `load_production_handoff()` which checks Key 2 (manifest `handoff_enabled` gate). |
| **Publication Mode** | A publisher-level parameter (`--mode` CLI argument, `publication_mode` field in manifest and run log) controlling how the publisher writes the manifest and active universe. Two valid modes: `validation_only` (default — writes `handoff_enabled=false`, `handoff_mode=production_validation`; observer gate accumulates) and `controlled_activation` (writes `handoff_enabled=true`, `handoff_mode=live`; activates second key of two-key gate). Not to be confused with `handoff_mode` (the manifest field the reader validates against allowed values). |
| **validation_only Mode** | The default publisher mode (`python3 handoff_publisher.py` or `--mode validation_only`). Writes `handoff_enabled=false` in the manifest. The reader always returns `fail_closed_reason="handoff_disabled_in_manifest"` for these manifests regardless of the config flag. Publisher run log accumulates toward the observation gate. No manual manifest edit is possible to activate while in this mode. |
| **controlled_activation Mode** | The publisher mode that sets Key 2 of the two-key activation gate (`python3 handoff_publisher.py --mode controlled_activation`). Writes `handoff_enabled=true` and `handoff_mode="live"` in the manifest. Reader accepts this manifest when the config flag (Key 1) is also True. Must be run immediately before the config flag flip in the activation sequence. For rollback: run `python3 handoff_publisher.py` (default `validation_only`) to reset `handoff_enabled=false`. |
| **handoff_mode** | A manifest field that the reader validates against allowed values `("paper", "live", "shadow")`. Set by the publisher based on `publication_mode`: `validation_only` → `production_validation` (not in reader's accepted set — reader rejects if it reaches this check, but `handoff_enabled=false` check fires first); `controlled_activation` → `"live"` (accepted by reader). |
| **Programmatic Activation** | The design principle (Sprint 7J.1) that the manifest gate (`handoff_enabled=true`) is always written by the publisher under a specific mode — never by manual JSON editing. This ensures the publisher validation cycle runs before any activation occurs, atomic write policy is preserved, and no path exists to an un-validated `handoff_enabled=true` manifest. |

## 20. Sprint 7J.3 — Observer Mode-Awareness Terms

These terms are added in Sprint 7J.3 to define the mode-aware observer design.

| Term | Definition |
|------|-----------|
| **Mode-Aware Observer** | The observer design (Sprint 7J.3) in which `handoff_publisher_observer.py` reads the current `publication_mode` from the live manifest at runtime and validates all publisher outputs against the expected state for that mode. Prior to Sprint 7J.3, the observer was hardcoded to `validation_only` and falsely flagged `controlled_activation` manifests as publisher errors. |
| **`_get_current_mode()`** | A pure read function in the observer that returns the current publication_mode from the live manifest. Returns `"validation_only"` as the safe default if the manifest is missing or unreadable. Returns `"unknown"` if the mode is present but not in `_VALID_PUBLICATION_MODES`. Never modifies any file. |
| **`_build_mode_context()`** | A function in the observer that builds the `mode_context` section of the observation report. Captures the manifest's publication mode, whether the manifest allows handoff, the bot config flag state (`enable_active_opportunity_universe_handoff`), whether both keys are active (`bot_consumption_allowed`), and an explanatory note (`bot_consumption_note`). Reads config at runtime but never modifies it. |
| **mode_context** | A section in the observation report (Sprint 7J.3) that captures the mode state of the publisher outputs. Contains: `publication_mode`, `handoff_enabled`, `mode_interpretation`, `expected_handoff_enabled_for_mode`, `manifest_mode_valid`, `manifest_allows_handoff`, `bot_flag_state`, `bot_consumption_allowed`, `bot_consumption_note`, `candidate_count`, `executable_violations`, `order_instruction_violations`. |
| **manifest_allows_handoff** | A boolean in `mode_context` that is `true` only when `publication_mode == "controlled_activation"` AND `handoff_enabled == true`. Indicates that the manifest (Key 2) is set for bot consumption. Does NOT mean the bot is consuming — that requires `bot_consumption_allowed` (both keys set). |
| **bot_consumption_note** | A string field in `mode_context` explaining why bot consumption is or is not active. Values: `"manifest_ready_but_bot_flag_disabled"` (manifest in controlled_activation but config flag=False), `"controlled_activation_manifest_handoff_not_enabled"` (mode is controlled_activation but handoff_enabled unexpectedly false), `"validation_only_manifest_handoff_disabled"` (normal validation_only state). Null if both keys are active. |
| **`controlled_activation_ready`** | A readiness gate value (Sprint 7J.3) yielded by the observer when `publication_mode=controlled_activation`, all publisher checks pass, and freshness SLA is met. Equivalent to `validation_only_stable` but for controlled_activation mode. The pre-activation check gate. |
| **`controlled_activation_unstable`** | A readiness gate value (Sprint 7J.3) yielded by the observer when `publication_mode=controlled_activation` but freshness SLA is not met (expired files). Triggers rollback consideration if seen during an active activation window. |
