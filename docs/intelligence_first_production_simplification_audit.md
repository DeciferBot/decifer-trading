...
## Sprint 7G File Classifications

**Sprint 7G — Validation-Only Live Observation with Publisher Running**
**Objective:** Create a read-only observer that analyses the 4 Sprint 7F publisher output files, detects freshness/validity/fail-closed events, and determines the readiness gate for flag activation. No production handoff triggered. handoff_enabled=false. live_bot_consuming_handoff=false.

### Created / Modified Files

| File | Classification | Included in live-bot container? | Notes |
|------|---------------|--------------------------------|-------|
| `handoff_publisher_observer.py` | Advisory/shadow-only (read-only observer) | No | Net-new. Reads publisher outputs → writes observation report. No imports of bot_trading/scanner/orders_core/guardrails/bot_ibkr/apex_orchestrator. |
| `data/live/handoff_publisher_observation_report.json` | Advisory/shadow-only (output) | No | Written atomically. mode=validation_only_handoff_publisher_observation, readiness_gate=insufficient_observation. |
| `data/live/.fail_*.json` | Diagnostic artefact — observation evidence | No | Written by handoff_publisher.py on any cycle failure. Read by observer for fail_closed_observations analysis. **Never read by live bot.** Retained during validation-only observation phase. See retention policy below. |
| `intelligence_schema_validator.py` | Advisory/shadow-only (tooling) | No | Extended with `validate_handoff_publisher_observation_report` wired into `validate_all()`. Now validates 39 files total (was 38 in Sprint 7F). |
| `tests/test_handoff_publisher_observer.py` | Test — advisory-only | No | Net-new. 48 tests, 10 groups. |

### `.fail_*.json` Diagnostic Retention Policy

| Property | Value |
|----------|-------|
| **Written by** | `handoff_publisher.py` — on any cycle failure (source missing, validation error, write error) |
| **Read by** | `handoff_publisher_observer.py` — observer counts and surfaces them in `fail_closed_observations` |
| **Live bot reads these?** | **No. Never.** The live bot reads only `data/live/current_manifest.json` and `data/live/active_opportunity_universe.json`. `.fail_*` files are explicitly excluded from any bot read path. |
| **Retention — validation-only phase** | Retain. These are evidence artefacts showing publisher failure modes. The observer reads them to assess publisher reliability before flag activation. |
| **Retention — pre-cloud production** | Must be resolved before cloud deployment. Options: (1) periodic rotation — delete files older than N days via a scheduled cleanup job; (2) exclude the glob from the production container image (`.dockerignore`); (3) rotate into a separate `data/live/diagnostics/` directory outside the primary handoff read path. **Do not accumulate indefinitely in the production container.** |
| **Do not delete now** | Retain during validation-only observation phase as evidence. Sprint 7G carries 4 diagnostic files from test runs — these are valid artefacts. |

### Validator Count — Reconciliation Note (Sprint 7F vs Sprint 7G)

The Sprint 7G end-of-sprint report stated `18/18 validate_intelligence_files`. This was a **reporting error** caused by using `tail -20` to view the validator output, which truncated the first 21 lines.

**Actual validator counts:**
- Sprint 7F: **38/38** (38 named validators wired into `validate_all()`)
- Sprint 7G: **39/39** — Sprint 7G adds 1 validator (`validate_handoff_publisher_observation_report`)

**No validation coverage was reduced.** All Sprint 7F-validated files (production active universe, current manifest, publisher report, publisher heartbeat, paper handoff artefacts, reference data files, factor registry files, intelligence files) continue to be validated. The "18" figure was the count of lines visible in a truncated terminal view, not the actual validator count.

Correct result: `python3 scripts/validate_intelligence_files.py` → **39/39 PASS**.

### Key Architecture Decisions Formalised in Sprint 7G

| Decision | Rationale |
|----------|-----------|
| Observer reads `.fail_*.json` glob from `data/live/` | Captures publisher failure events without modifying publisher; glob pattern isolates diagnostics from primary outputs |
| `.fail_*.json` files are diagnostic artefacts, not runtime inputs | Live bot must never read them; they inform the observer only during validation-only phase |
| `candidate_stability_analysis` is single-observation-only at gate start | Stability tracking requires ≥2 observations; single run returns `status="single_observation"` — no false stability claims |
| Readiness gate `insufficient_observation` until ≥10 successful publisher runs | Prevents premature flag activation based on insufficient evidence |
| Observer never imports publisher | Clean separation — observer cannot accidentally trigger a publish cycle |

### Sprint 7G Anti-Bloat Confirmation
...
| live_output_changed | False |
| .fail_*.json read by live bot? | No — diagnostic artefacts only |

---

## Sprint 7H File Classifications

**Sprint 7H — Handoff Activation Design Review**
**Objective:** Produce the activation design review, activation checklist, and controlled activation test plan. No code changes. No flag flip. Production activation remains blocked pending Amit approval.

### Created Files

| File | Classification | Included in live-bot container? | Notes |
|------|---------------|--------------------------------|-------|
| `docs/intelligence_first_handoff_activation_design_review.md` | Advisory/design document | No | 8-section review: current state, evidence, 8 risks, 4 options, prerequisites, window, rollback, go/no-go. |
| `docs/intelligence_first_handoff_activation_checklist.md` | Advisory/design document | No | 14-section checklist with Amit approval field. To be executed in activation sprint. |
| `docs/intelligence_first_controlled_activation_test_plan.md` | Advisory/design document | No | 78 test cases across 14 groups. Tests not yet implemented — design artefact for activation sprint. |

### Session Diversity Limitation

All 10 successful publisher runs that advanced the gate to `validation_only_stable` occurred on 2026-05-07 within a 38-minute window. The `distinct_utc_sessions = 1` criterion was not met. This is documented as RISK-01 (HIGH) in the design review. The recommended action (Option A) is to continue observation until `distinct_utc_sessions >= 3` before entering the activation sprint.

### Sprint 7H Anti-Bloat Confirmation
...
| Risk/order/execution logic changed? | No |
| live_output_changed | False |
| Tests implemented? | No — test plan is a design artefact only |

---

## Sprint 7G.1 File Classifications

**Sprint 7G.1 — Publisher Observation Run Log Patch**
**Objective:** Fix a structural bug where `_count_publisher_runs()` was permanently capped at `successful=1` (heartbeat stores only the latest run). Root fix: append-only JSONL run log in the publisher; observer reads all lines for accurate dual-threshold counting.

### Created / Modified Files

| File | Classification | Included in live-bot container? | Notes |
|------|---------------|--------------------------------|-------|
| `handoff_publisher.py` | Production runtime candidate | No (validation_only phase) | Extended: `_append_run_log()` appends one JSONL line per successful cycle after all 4 primary outputs written. Fail-safe: OS error warns, does not corrupt manifest. |
| `handoff_publisher_observer.py` | Advisory/shadow-only (read-only observer) | No | Refactored: `_count_publisher_runs()` → `_read_run_log()`. `_determine_readiness_gate()` now accepts `run_log` dict, returns `(gate, threshold_met, threshold_basis)` tuple. Dual-threshold: threshold_met = (runs≥10 OR sessions≥3). |
| `data/live/publisher_run_log.jsonl` | Production observability output | No | Append-only. One JSONL line per successful publisher cycle. Persists across restarts. Required fields: schema_version, run_id, worker, completed_at, utc_date, validation_status, publication_mode, handoff_enabled=false, enable_active_opportunity_universe_handoff=false, candidate_count, manifest_expires_at, safety_flags (all false), live_output_changed=false. |
| `intelligence_schema_validator.py` | Advisory/shadow-only (tooling) | No | Extended: `validate_publisher_run_log()` added (13 checks per line); `validate_handoff_publisher_observation_report()` updated for new observation_summary fields; Sprint 7G.1 block in `validate_all()`. Now validates **40 files** (was 39 in Sprint 7G). |
| `tests/test_handoff_publisher.py` | Test — production runtime | No | Extended with Group 11 (10 tests): run log creation, append, multi-append, fail-safe, required fields, safety flags, validator accept, validator reject malformed/handoff_enabled_true/live_output_changed_true. |
| `tests/test_handoff_publisher_observer.py` | Test — advisory-only | No | Extended with Group 11 (11 tests): run_log counting, distinct sessions, gate thresholds, gate advance (runs/sessions), publisher issues block advance, missing log, heartbeat health, live bot isolation, safety flags. |

### `publisher_run_log.jsonl` Retention Policy

| Property | Value |
|----------|-------|
| **Written by** | `handoff_publisher.py` — appended after each successful publish cycle |
| **Read by** | `handoff_publisher_observer.py` — reads all lines to count successful runs and distinct sessions |
| **Live bot reads this?** | **No. Never.** The live bot reads only `data/live/current_manifest.json` and `data/live/active_opportunity_universe.json`. The run log is an observability output, not a bot input. |
| **Retention — validation-only phase** | Retain. All lines are evidence. The observer reads the full history across restarts. |
| **Retention — pre-cloud production** | Must be handled before cloud deployment. Same requirements as `.fail_*.json`: (1) periodic rotation — delete/archive lines older than N days; (2) exclude from production container image (`.dockerignore`); (3) cap file size — rotate when file exceeds a size threshold. File grows unboundedly at ~1 line per publisher cycle otherwise. |

### Sprint 7G.1 Gate Logic Update

| Gate value | Condition |
|------------|-----------|
| `insufficient_observation` | successful_runs < 10 AND distinct_sessions < 3 |
| `fix_publisher_before_flag_activation` | threshold_met AND manifest/universe/heartbeat issues |
| `validation_only_unstable` | threshold_met AND freshness expired |
| `validation_only_stable` | threshold_met AND all analyses pass |

Note: `ready_for_flag_activation_design` gate removed. It was never reachable and conflated readiness assessment with Sprint 7H design work. The correct gate for a clean observation run above threshold is `validation_only_stable`.

### Sprint 7G.1 Anti-Bloat Confirmation
...
| Risk/order/execution logic changed? | No |
| live_output_changed | False |
| publisher_run_log.jsonl read by live bot? | No — observability output only |

---

## Sprint 7H.1 File Classifications

**Sprint 7H.1 — Activation Readiness Operations Pack**
**Objective:** Create 6 new documentation files and update 3 existing files to complete the operations and activation readiness pack. No production code changed. No handoff flag flipped.

### Created Files

| File | Classification | Included in live-bot container? | Notes |
|------|---------------|--------------------------------|-------|
| `docs/intelligence_first_cloud_deployment_runbook.md` | Advisory/design document | No | Phase 1 deployment model, process list, required dirs, secrets policy, IBKR model, healthchecks, rollback steps |
| `docs/intelligence_first_activation_rollback_playbook.md` | Advisory/design document | No | Pre-activation conditions, activation window, activation steps, rollback steps, rollback triggers, post-activation review |
| `docs/intelligence_first_daily_operator_checklist.md` | Advisory/design document | No | Pre-market / during-market / post-market / emergency sections |
| `docs/intelligence_first_log_retention_and_diagnostics_policy.md` | Advisory/design document | No | Artefact classification, retention policy, container inclusion/exclusion, live bot read policy |
| `docs/intelligence_first_snapshot_archive_design.md` | Advisory/design document | No | Deferred design — implementation gated on activation sprint completion |
| `docs/intelligence_first_scanner_only_removals_review_plan.md` | Advisory/design document | No | Review categories A–F, review inputs, output template, activation gate |

### Modified Files

| File | Change | Classification |
|------|--------|---------------|
| `docs/intelligence_first_definitions_and_runtime_contract.md` | Section 17 added — 9 activation readiness terms | Advisory/design document |
| `docs/intelligence_first_retirement_register.md` | Sprint 7H.1 entry added | Advisory/design document |
| `docs/intelligence_first_production_simplification_audit.md` | Sprint 7H.1 classifications and update log entry added | Advisory/design document |

### Files Explicitly NOT Modified in Sprint 7H.1

| File | Reason |
|------|--------|
| `bot_trading.py` | Locked — no changes |
| `config.py` | Locked — no flag changes |
| `handoff_publisher.py` | Locked — no changes |
| `handoff_publisher_observer.py` | Locked — no changes |
| `intelligence_schema_validator.py` | Locked — no changes |
...
| Any test file | Locked — documentation sprint only |

### Sprint 7H.1 Anti-Bloat Confirmation
...
## Sprint 7H.2 File Classifications

**Sprint 7H.2 — Scanner-Only Removals Review**
**Objective:** Classify all 208 scanner-only symbols (symbols present in scanner universe but absent from governed shadow universe) before activation. Review and classification only — no symbol approvals, no roster changes, no production code changes.

### Created Files

| File | Classification | Included in live-bot container? | Notes |
|------|---------------|--------------------------------|-------|
| `data/intelligence/scanner_only_removals_review.json` | Advisory/classification output | No | 160 symbol entries across 7 categories; schema v1.0; mode=review_only |
| `docs/intelligence_first_scanner_only_removals_review_summary.md` | Advisory/design document | No | 8-section human-readable summary; activation gate assessment |

### Files Explicitly NOT Modified in Sprint 7H.2

| File | Reason not modified |
|------|---------------------|
| `data/reference/thematic_roster.json` | No symbols approved — review only |
| `universe_builder.py` | No changes — review only |
| `candidate_resolver.py` | No changes — review only |
| `bot_trading.py` | No changes |
| `scanner.py` | No changes |
| `guardrails.py` | No changes |
| `config.py` | No changes — flag remains False |
| `docs/intelligence_first_retirement_register.md` | Sprint 7H.2 entry added only |
| `docs/intelligence_first_production_simplification_audit.md` | Sprint 7H.2 entry added only |

### Sprint 7H.2 Anti-Bloat Confirmation
...
| live_output_changed | False |
| Symbols approved? | No |
| thematic_roster.json changed? | No |

---

## Sprint 7J.1 File Classifications

**Sprint 7J.1 — Two-Key Activation Gate Patch**
**Objective:** Implement a publisher mode system so `controlled_activation` mode writes `handoff_enabled=true` programmatically, enabling the two-key activation model. No flag flip. No live bot consumption.

### Modified Files

| File | Change | Classification |
|------|--------|----------------|
| `handoff_publisher.py` | `_VALID_PUBLICATION_MODES`, `_DEFAULT_PUBLICATION_MODE`, `_HANDOFF_MODES` constants; `--mode` argparse; mode parameter wired through all builders/validators; `_append_run_log()` records actual `publication_mode` and `handoff_enabled` per run | Production runtime (publisher) |
| `intelligence_schema_validator.py` | `validate_prod_active_universe`, `validate_prod_manifest`, `validate_handoff_publisher_report`, `validate_publisher_run_log` updated to be mode-aware (accept both `validation_only` and `controlled_activation`) | Production observability |

### Created Files

| File | Classification |
|------|----------------|
| `tests/test_handoff_activation_gate.py` | Production observability test (20 tests) |

### Updated Tests

| File | Change |
|------|--------|
| `tests/test_handoff_publisher_observer.py` | `test_readiness_gate_is_insufficient_observation` → `test_readiness_gate_is_valid` (gate set broadened) |

### Files Explicitly NOT Modified in Sprint 7J.1

| File | Reason not modified |
|------|---------------------|
| `bot_trading.py` | No change — live bot still scanner-led; `enable_active_opportunity_universe_handoff=False` |
| `handoff_reader.py` | No change — reader already accepts `handoff_mode in ("paper", "live", "shadow")`; `"live"` is the correct value for `controlled_activation` |
| `config.py` | `enable_active_opportunity_universe_handoff` remains `False` — flag flip requires Amit explicit approval |
| `scanner.py` | No change |
| `guardrails.py` | No change |
| `apex_orchestrator.py` | No change |

### Sprint 7J.1 Anti-Bloat Confirmation
...
| New .env reads? | No |
| Production handoff flag changed? | No — `enable_active_opportunity_universe_handoff = False` |
| handoff_enabled in manifest (default mode)? | False (validation_only) |
| publication_mode (default)? | validation_only |
| Live bot reads publisher outputs? | No — flag is False |
| bot_trading.py modified? | No |
| scanner.py modified? | No |
| guardrails.py modified? | No |
...
| Risk/order/execution logic changed? | No |
| live_output_changed | False |
| Two-key model: both keys required? | Yes — config flag AND manifest `handoff_enabled=true` both required |

---

## Sprint 7I File Classifications

**Sprint 7I — Quota Policy Promotion to 75/35 (Validation-Only)**
**Objective:** Promote the handoff universe quota from 50/20 (safety validation cap) to 75/35 (production candidate cap) in validation-only mode. No handoff flag flip. No live bot consumption. Evidence: Sprint 7H.3 calibration report.

### Modified Production Runtime Files

| File | Change | Classification |
|------|--------|----------------|
| `quota_allocator.py` | `_TOTAL_MAX` 50→75, `_STRUCTURAL_MAX` 20→35, `QUOTA_POLICY_VERSION="75_35"` added | Production runtime (shadow pipeline) |
| `universe_builder.py` | `_QUOTA`, `_TOTAL_MAX`, `_STRUCTURAL_MAX`, `_ATTENTION_MAX`, `_ETF_PROXY_MAX` updated | Production runtime (shadow pipeline) |
| `intelligence_schema_validator.py` | `_TOTAL_MAX` 50→75, `_QUOTA_CAPS["structural_position"]` 20→35, Sprint 7I quota checks added | Production observability |
| `handoff_publisher.py` | `_quota_policy_version()` helper added; `quota_policy_version` written to run log entries | Production runtime (publisher) |
| `handoff_publisher_observer.py` | Quota-policy-aware run counts, watch symbol tracking (COST/MSFT/PG/SNDK/WDC/IREN), `structural_count` | Production observability |

### Created Files

| File | Classification |
|------|----------------|
| `tests/test_quota_policy_promotion.py` | Production observability test (18 tests) |

### Files Explicitly NOT Modified in Sprint 7I

| File | Reason not modified |
|------|---------------------|
| `bot_trading.py` | No change — live bot still scanner-led |
| `scanner.py` | No change |
| `guardrails.py` | No change |
| `config.py` | No change — `enable_active_opportunity_universe_handoff = False` unchanged |
| `thematic_roster.json` | No change — no symbols approved |
| `candidate_resolver.py` | No change |
| `apex_orchestrator.py` | No change |

### Sprint 7I Anti-Bloat Confirmation
...
| Symbols approved? | No |
| thematic_roster.json changed? | No |
| Second bot created? | No — same pipeline, updated quota constants only |
...
## Closure Sprint Additions (2026-05-11)

The following files were created or modified in the closure sprint. Each is classified below.

| File | Classification | Service Layer | Production Import | Cloud Include | Notes |
|------|---------------|---------------|------------------|---------------|-------|
| `scripts/cloud_preflight.py` | `cloud_deployment` | Cloud / Deployment | No (script only) | Yes | Standalone preflight check; safe to run anywhere; never touches broker or orders |
| `scripts/intelligence_first_paper_validation_report.py` | `reporting_advisory` | Reporting / Advisory | No (script only) | Optional | Reads-only from logs; produces JSON + MD report; never touches execution |
| `Dockerfile` | `cloud_deployment` | Cloud / Deployment | No | Yes | Minimal build; TA-Lib from source; default CMD = preflight check (no trading) |
| `.dockerignore` | `cloud_deployment` | Cloud / Deployment | No | Yes | Excludes secrets, data/, logs/, .git/ from build context |
| `ops/launchd/com.decifer.handoff-publisher.plist` | `cloud_deployment` | Intelligence Worker / Scheduler | No | Reference only | Publisher schedule template; StartInterval=600; controlled_activation mode |
| `bot_ibkr.py` (4 exception handlers) | `production_runtime` | Live Bot / IBKR | Yes | Yes | Silent `except: pass` → structured `log.debug()` calls in backfill dedup paths; no behaviour change |
| `signal_types.py` (5 new fields) | `production_runtime` | Signal Pipeline | Yes | Yes | Added optional `handoff_source_labels`, `handoff_route`, `handoff_reason_to_care`, `handoff_freshness_status`, `handoff_candidate_id` to Signal dataclass; backward-compatible |
| `signal_pipeline.py` (`governance_map` param) | `production_runtime` | Signal Pipeline | Yes | Yes | Added optional `governance_map` kwarg to `run_signal_pipeline` and `_scored_to_signals`; no change when map is None |
| `bot_trading.py` (`governance_map` kwarg) | `production_runtime` | Live Bot / Trading | Yes | Yes | Passes `_handoff_governance_map` to `run_signal_pipeline`; None when handoff not active |
| `docs/intelligence_first_architecture_proof_matrix.md` | `docs_only` | Documentation | No | No | 37-row proof matrix for closure sprint |
| `docs/intelligence_first_paper_validation_report.md` | `docs_only` | Documentation | No | No | Auto-generated by paper validation report script |
| `data/runtime/` (directory) | `runtime_data` | Runtime State | No (data only) | Yes | New runtime output directory for preflight and report outputs |

### Simplification Risks Introduced

None. All changes are:
- Additive (new optional fields/params with None default)
- Purely observational (debug logging, report generation)
- Documentation/deployment files (no production import)

### Simplification Risks Removed

| Risk | How Removed |
|------|------------|
| Silent exception swallowing in IBKR backfill dedup | 4 `except Exception: pass` replaced with `log.debug()` — failures now visible in bot log |
| Handoff source labels missing from signals audit log | Signal dataclass updated; provenance preserved in signals_log.jsonl from first scan cycle post-merge |
| No cloud deploy validation gate | `cloud_preflight.py` now provides structured go/no-go for any new environment |
| No paper validation paper trail | Report script produces evidence for Q1-Q10 at any time |

### Modules That Must NOT Be Imported in Production

No changes to the existing prohibited list. The following from the prior audit remain excluded:

- `backtest_intelligence.py` — backtest only
- `advisory_reporter.py` — offline report tool
- `advisory_log_reviewer.py` — offline review tool
- `intelligence_schema_validator.py` — CI/CD only

### Retirement Register — No Changes

No retirements triggered in this sprint. All 7 removal gate conditions remain required before any module is deleted.
...
| Date | Action | Notes |
|------|--------|-------|
| 2026-05-24 | Sprint M6 — Production Hardening | **saas_intelligence_output.py**: `validate_customer_payload()` strengthened from 2 rules to 7 — added broker-like field name check, execution-wording-in-values check, internal-artefact-name check, `data_entitlement_note` required, `freshness_timestamp` age gate (6h window). Imports `datetime` (new). No new public surface — module exports unchanged. **market_now_builder.py**: `_is_degraded()` helper added; `build_market_now()` gains explicit degraded path — stale/missing artefacts produce "market intelligence temporarily limited" response with `confidence_label=Insufficient data` and fresh timestamp; never hallucinate missing state. `import time` added. No new module. **intelligence_api.py**: `/health` expanded from 5 fields to 10 — adds `customer_output_mode`, `data_freshness_status`, `latest_market_now_timestamp`, `latest_pipeline_artifact_timestamp`, `degraded_artifact_warnings`. `_build_health_freshness()` helper added (reads artefact mtimes; exposes timestamps only — no file paths, no secrets, no broker state). `import json`, `import time`, `_BASE_DIR` constant added. No new routes. **scripts/verify_intelligence_cloud_production_hardening.py**: NEW — verifier/test-only, 24 checks (H1–H7, M1–M4, V1–V6, E1–E2, R1–R3, D1, B1). Not imported at runtime. **docs**: INTELLIGENCE_CLOUD_PROOF.md + DIGITALOCEAN_INTELLIGENCE_CLOUD_DEPLOYMENT.md updated with Sprint M6 results and Cloudflare Access documentation. **Cloud runtime impact**: same 3 production modules (zero new), no new routes, expanded field set on `/health`. No execution modules touched. bot.py, orders_*.py: unchanged. Pre-existing verifier (14/14) and new M6 verifier (24/24) both pass. |
...
| 2026-05-11 | Closure Sprint | Classified 12 new/modified files. 4 silent exception handlers fixed. handoff metadata propagation added to Signal + signal_pipeline. Cloud preflight + Dockerfile + paper validation report created. Publisher plist template added. |
...
| 2026-05-07 | Sprint 7G | Validation-Only Live Observation with Publisher Running. handoff_publisher_observer.py (read-only observer, advisory/shadow-only) net-new. Reads 4 Sprint 7F publisher output files + .fail_*.json diagnostics; writes data/live/handoff_publisher_observation_report.json atomically. 8 analysis sections: observation_summary (runs: 1ok/3fail), freshness_analysis (SLA: 10/15/20min, sla_met=true), manifest_validity_analysis (handoff_enabled=false, validation_only, safety_flags_clean), active_universe_validity_analysis (50 candidates, 0 violations), heartbeat_analysis (pass, fresh), candidate_stability_analysis (single_observation mode), fail_closed_observations (3 events surfaced from test runs), safety_analysis (all_safety_invariants_hold=true). Readiness gate: insufficient_observation (1 successful run vs 10 threshold). Observer never imports publisher. validate_handoff_publisher_observation_report added to intelligence_schema_validator.py; wired into validate_all(). validate_intelligence_files.py now validates **39 files** (Sprint 7F=38 + 1 new). NOTE: end-of-sprint report incorrectly stated "18/18" — this was a tail-truncation reporting error; actual coverage was never reduced (see Validator Count Reconciliation Note in Sprint 7G File Classifications section). .fail_*.json files are diagnostic artefacts: never read by live bot, retained during validation-only phase, must be rotated/excluded before cloud production. tests/test_handoff_publisher_observer.py net-new, 48 tests. No bot_trading.py modified. No scanner.py. No guardrails.py. No Apex. No risk/order/execution changes. live_bot_consuming_handoff=false. enable_active_opportunity_universe_handoff=False. live_output_changed=false. 48/48 observer, 356/356 Sprint 7 combined, **39/39 validator**, 9/9 smoke. |
| 2026-05-07 | Sprint 7G.1 | Publisher Observation Run Log Patch. Root cause: `_count_publisher_runs()` capped at successful=1 (heartbeat stores only latest run); neither 10-run nor 3-session threshold reachable. Fix: `_append_run_log()` added to publisher (appends one JSONL line per successful cycle to `data/live/publisher_run_log.jsonl`); fail-safe on OS error (warns, returns False, does not corrupt manifest). Observer refactored: `_read_run_log()` reads all lines; dual-threshold gate: threshold_met = (runs≥10 OR sessions≥3), threshold_basis = "successful_runs" \| "distinct_sessions" \| "not_met"; gate `ready_for_flag_activation_design` removed → `validation_only_stable`. Observation_summary extended with run_log_exists, successful_publisher_runs, distinct_utc_sessions, threshold_met, threshold_basis. Validator extended: validate_publisher_run_log() (13 checks per line), observation_report validator updated; validate_all() now checks 40 files. `publisher_run_log.jsonl` classification: production observability output — never read by live bot; same rotation requirements as .fail_*.json before cloud production. Tests: 10 new in test_handoff_publisher.py (Group 11), 11 new in test_handoff_publisher_observer.py (Group 11). No bot_trading.py modified. No scanner.py. No guardrails.py. No Apex. live_output_changed=false. enable_active_opportunity_universe_handoff=False. **117/117 publisher, 59/59 observer, 377/377 Sprint 7 combined, 40/40 validator, 9/9 smoke. Gate: insufficient_observation (3 runs < 10, 1 session < 3).** |
| 2026-05-09 | Sprint 7J.1 | Two-Key Activation Gate Patch — publisher mode system implemented. No flag flip. No live bot consumption. Root cause: prior single-key design had reader gating on manifest `handoff_enabled`, which was always `false` in validation_only mode — flag alone insufficient to activate. Fix: publisher gains `--mode {validation_only,controlled_activation}` (default: validation_only). `controlled_activation` mode writes `handoff_enabled=true` and `handoff_mode=live` (reader's accepted value) without any manual manifest edit. Both keys must be set for live bot consumption: (1) `enable_active_opportunity_universe_handoff=True` in config.py + (2) `handoff_enabled=true` in manifest written by `--mode controlled_activation`. Files changed: `handoff_publisher.py` (constants `_VALID_PUBLICATION_MODES`, `_DEFAULT_PUBLICATION_MODE`, `_HANDOFF_MODES`; mode param wired through `run_publisher()`, `_build_manifest()`, `_build_active_universe()`, `_build_publisher_report()`, `_validate_output_universe()`, `_validate_output_manifest()`, `_append_run_log()`, `_fail_closed_cycle()`; argparse `--mode` in `__main__`); `intelligence_schema_validator.py` (4 validators updated to be mode-aware: `validate_prod_active_universe`, `validate_prod_manifest`, `validate_handoff_publisher_report`, `validate_publisher_run_log`). Tests: `tests/test_handoff_activation_gate.py` net-new (20 tests — publisher default mode, validation_only/controlled_activation manifest correctness, reader acceptance/rejection, no executable candidates, no order instructions, 75 candidates, quota_policy_version, fail-closed on bad shadow source, stale manifest, rollback to validation_only). `tests/test_handoff_publisher_observer.py` — 1 test updated (`test_readiness_gate_is_insufficient_observation` → `test_readiness_gate_is_valid`, now checks gate against full valid gate set). No bot_trading.py modified. No scanner.py. No guardrails.py. No Apex. No risk/order/execution changes. `enable_active_opportunity_universe_handoff = False` (config flag remains False — activation requires Amit explicit approval). `live_output_changed = false`. **295/295 handoff-suite tests, 9/9 smoke, 40/40 validator.** |
| 2026-05-09 | Sprint 7J.2 | Controlled Handoff Activation Attempt — NO-GO. No production code changed. No config flag flipped. Two-key sequence partially executed: Key 2 (`python3 handoff_publisher.py --mode controlled_activation`) ran successfully — publisher wrote `handoff_enabled=true`, `handoff_mode=live`, 75 candidates accepted, all pass/40 validator, all safety flags clean. Config flag (Key 1) was NOT flipped. Root cause of NO-GO: `handoff_publisher_observer.py` triggered `gate=fix_publisher_before_flag_activation` — false alarm. Observer's `_analyse_manifest()` was hardcoded to expect `handoff_enabled=false` and `publication_mode=validation_only` (lines 249, 251) — it treated the correctly written `controlled_activation` manifest as a publisher error. This was an observer design gap, not a publisher failure. Two-key sequence rolled back correctly: `python3 handoff_publisher.py` (default validation_only) reset manifest to `handoff_enabled=false`. Sprint 7J.3 raised to fix observer. No bot_trading.py modified. No scanner.py. No guardrails.py. No Apex. No risk/order/execution changes. `enable_active_opportunity_universe_handoff = False` throughout. `live_output_changed = false`. |
| 2026-05-09 | Sprint 7J.3 | Observer Controlled-Activation Awareness Patch. No flag flip. No live bot consumption. Root cause of Sprint 7J.2 NO-GO: `handoff_publisher_observer.py` hardcoded `_PUBLICATION_MODE = "validation_only"` and checked `manifest.get("handoff_enabled") is not False` (false for controlled_activation mode) — observer treated `handoff_enabled=true` as a publisher error regardless of mode. Fix: observer is now mode-aware. New constants: `_VALID_PUBLICATION_MODES = {"validation_only", "controlled_activation"}`, `_MODE_EXPECTED_HANDOFF_ENABLED`, `_MODE_INTERPRETATION`. New functions: `_get_current_mode()` (reads manifest at runtime, defaults to `"validation_only"`, returns `"unknown"` for unrecognised modes), `_build_mode_context()` (returns 12-field `mode_context` section: `publication_mode`, `handoff_enabled`, `mode_interpretation`, `expected_handoff_enabled_for_mode`, `manifest_mode_valid`, `manifest_allows_handoff`, `bot_flag_state`, `bot_consumption_allowed`, `bot_consumption_note`, `candidate_count`, `executable_violations`, `order_instruction_violations`). All analysis functions (`_analyse_manifest`, `_analyse_active_universe`, `_analyse_publisher_report`) gain `expected_mode` parameter. `_determine_readiness_gate()` gains `current_mode` parameter — `controlled_activation` mode yields `controlled_activation_ready` / `controlled_activation_unstable`; `fix_publisher_before_flag_activation` fires only for real errors (bad candidates, safety flags dirty, missing heartbeat) not mode mismatch. `_SAFETY` dict: `handoff_enabled` and `publication_mode` removed (now dynamic); all other safety invariants remain hardcoded. `intelligence_schema_validator.py` updated: `_OBSERVATION_SAFETY_FLAGS_MUST_BE_FALSE` no longer checks `handoff_enabled` (mode-dependent); `_VALID_OBSERVATION_READINESS_GATES` adds `controlled_activation_ready`, `controlled_activation_unstable`; `validate_handoff_publisher_observation_report()` now mode-aware for `publication_mode`/`handoff_enabled` combination check and warns if `mode_context` fields missing. `tests/test_handoff_publisher_observer.py` extended with `TestModeAwareness` class (18 tests across Group 12): validation_only/controlled_activation manifest acceptance/rejection, mode_context fields, gate names, live_output_changed=false, no broker/LLM/bot consumption, validator acceptance of both modes, smoke. No bot_trading.py modified. No scanner.py. No guardrails.py. No Apex. No risk/order/execution changes. `enable_active_opportunity_universe_handoff = False`. `live_output_changed = false`. **76/76 observer tests, 9/9 smoke, 40/40 validator.** |
| 2026-05-07 | Sprint 7I | Quota Policy Promotion to 75/35 — validation-only mode. No handoff flag flip. No live bot change. Quota constants promoted from 50/20 (safety validation cap) to 75/35 (production candidate cap). Files changed: quota_allocator.py, universe_builder.py, intelligence_schema_validator.py, handoff_publisher.py (run log now records quota_policy_version), handoff_publisher_observer.py (quota-policy-aware gate, watch symbol tracking). Shadow universe regenerated: 75 candidates, 35 structural. COST/MSFT/PG/SNDK/WDC/IREN all included. Observation gate reset: 1/10 successful runs for 75/35 policy as of sprint close. **18/18 quota promotion tests, 40/40 validator, 9/9 smoke.** |
| 2026-05-07 | Sprint 7H.2 | Scanner-Only Removals Review — advisory/classification sprint only. No production code changed. No symbols approved. No thematic_roster.json changes. No universe_builder.py or candidate_resolver.py changes. No handoff flag flipped. 160 of 208 scanner-only symbols classified across 7 categories: 59 scanner_only_attention, 55 future_theme_candidate, 18 review_required, 19 unknown_requires_provider_enrichment, 3 rejected, 3 governance_gap_defect (COST/MSFT/PG — EIL-governed but quota-excluded; not a pipeline bug; requires Amit acknowledgement), 3 already_governed_elsewhere. 48 symbols below tracking threshold implicitly classified scanner_only_attention. Two artefacts created: data/intelligence/scanner_only_removals_review.json and docs/intelligence_first_scanner_only_removals_review_summary.md. Activation not hard-blocked but requires Amit acknowledgement of governance gap defects. **40/40 validator, 9/9 smoke.** |
| 2026-05-07 | Sprint 7H.1 | Activation Readiness Operations Pack — documentation sprint only. No production code changed. No handoff flag flipped. Six new documents created: intelligence_first_cloud_deployment_runbook.md (Phase 1 deployment, startup/shutdown order, process list, required dirs, secrets policy, IBKR model, healthchecks, rollback steps), intelligence_first_activation_rollback_playbook.md (13 pre-activation conditions, activation window design, 15 activation steps, 14 rollback steps, 11 immediate + 3 deferred rollback triggers, 12-item post-activation review), intelligence_first_daily_operator_checklist.md (pre-market/during-market/post-market/emergency — 48 checks across 4 sections), intelligence_first_log_retention_and_diagnostics_policy.md (4 artefact classes, per-file retention policy, container inclusion/exclusion, live bot read policy), intelligence_first_snapshot_archive_design.md (deferred design — purpose, archive path, what to archive, retention, implementation gate), intelligence_first_scanner_only_removals_review_plan.md (known context, 6 review categories A-F, review inputs with diff code, output report template with activation gate). Three existing documents updated: intelligence_first_definitions_and_runtime_contract.md Section 17 added (9 activation readiness terms), retirement_register.md Sprint 7H.1 entry added, production_simplification_audit.md Sprint 7H.1 entry added. No new live API paths. No broker calls. No LLM calls. No .env reads. No scanner/guardrails/Apex/risk/order/execution changes. enable_active_opportunity_universe_handoff=False. live_output_changed=false. **40/40 validator, 9/9 smoke.** |
| 2026-05-07 | Sprint 7H | Handoff Activation Design Review — design sprint only. No production code changed. No handoff flag flipped. Gate validation_only_stable reached (10 runs, 40/40, 9/9). Session diversity NOT achieved (1 UTC session, not 3 — single-session limitation documented as HIGH risk). Three design documents created: docs/intelligence_first_handoff_activation_design_review.md (8-section review: current state, evidence, 8 risks, 4 options, prerequisites, activation window, rollback design, go/no-go criteria), docs/intelligence_first_handoff_activation_checklist.md (14-section checklist with Amit approval field), docs/intelligence_first_controlled_activation_test_plan.md (78 test case design across 14 groups — design artefact, not yet implemented). Key findings: RISK-01 single-session evidence HIGH; 208 scanner-only removals acknowledged; SNDK/WDC/IREN quota-excluded but governed; no snapshot archive; fail-closed by design (no scanner fallback); PM Track B independent. Recommendation: Option A — continue observation across ≥3 distinct UTC sessions. Production activation blocked — requires 13 Go criteria + Amit explicit approval. No bot_trading.py modified. No scanner, guardrails, Apex, risk, order, execution changes. enable_active_opportunity_universe_handoff=False. live_output_changed=false. **40/40 validator, 9/9 smoke. Full suite not run (documentation-only sprint). Production activation blocked.** |
