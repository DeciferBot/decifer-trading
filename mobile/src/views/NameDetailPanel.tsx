      ...
      >
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-2xl font-black text-slate-100">{name.symbol}</h2>
            {name.theme_link && (
              <span
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {themeDesc && name.theme_link && (
            <section>
              ...
              >
                <p className="text-[11px] font-bold mb-1.5" style={{ color: "#fb923c" }}>
                  {translateTheme(name.theme_link)}
                </p>
                <p className="text-xs text-slate-300 leading-relaxed">{themeDesc}</p>
              </div>
            </section>
          )}
              ...
              <ul className="space-y-1.5">
                {(data?.what_changed ?? []).slice(0, 3).map((item, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full mt-1.5 shrink-0 bg-slate-500" />
