...
- **Test suite**: 2836 passing (2026-05-20). 8 pre-existing failures (unrelated to this work). +20 new HMM tests.
- **Regime detector**: VIX-proxy + SPY EMA for `scanner.get_market_regime()` (locked for scanner until Phase B scanner replacement approved). HMM advisory active in signal weight router.
