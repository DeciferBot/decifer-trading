...
- **Position Research Universe Phase 1 — Shadow observation only ✅** (2026-05-03): Tier D discovery path. `universe_position.py` builds a broad discovery net (discovery points + archetypes) from the committed Master Universe, bypassing the gap/premarket-volume promoter. Scanner exposes Tier D via `get_position_research_universe()`. Signal pipeline tags Tier D BEFORE gates and bypasses persistence. Apex receives `[POSITION_CANDIDATE]` prefix for Tier D names. Shadow mode active (`position_research_shadow_mode=True`): full POSITION simulation runs, ctx enriched with data-source classification + data-flow gap detection, logged to `data/position_research_shadow.jsonl`. Backfill pass in `signal_dispatcher._backfill_tier_d_contexts()` retries FMP for no-fundamental Tier D ctx. Evidence script: `scripts/tier_d_evidence_report.py`. **Phase 2 gate: NOT MET** — need ≥10 distinct Tier D symbols evaluated across multiple real scan cycles. Do NOT soften gates, enable live entries, or use PRU snapshot for live validation until Phase 2 gate is met and Amit approves. 25 tests, all passing.
- **Test suite**: 2024 passing (2026-05-03). 2 pre-existing trailing stop failures unrelated to Phase 1.
