...
- **Position Research Universe Phase 1 — Shadow observation only ✅** (2026-05-03): Tier D discovery path. Full pipeline instrumentation in place: `tier_d_funnel.jsonl` records stage=pipeline (stages 1-6 attrition), stage=dispatch (Apex classification breakdown), and stage=apex_cap (top-30 hard cap before Apex — whether Tier D is being killed before Apex sees them). Evidence script: `scripts/tier_d_evidence_report.py`. **Phase 2 gate: WIRED ✅ (2026-05-31, v4.92.0).** Root cause fixed: PRU symbols were never added to the scoring universe. `bot_trading.py` now loads PRU tickers via `load_position_research_universe()` and merges them into `universe` before `run_signal_pipeline()`. `signal_pipeline.py` adds `_apply_tier_d_scoring_cap()` (ranks/caps PRU symbols before `score_universe()`, writes `scoring_cap` funnel record) and `_tag_tier_d()` (attaches `scanner_tier="D"` + discovery metadata post-scoring). PRU (150 symbols, built 2026-05-31) valid for 8 days. Next scan cycle will show non-zero `tier_d_discovery_pool_count`. Phase 2 decisions (tier-aware shortlist / scoring threshold / Apex prompt) can now be made from real evidence. 30 tests, all passing.
...
- **Earnings Transcript Intelligence — Active ✅** (2026-05-31): `earnings_transcript_engine.py` (INTELLIGENCE layer). After market close, the intelligence pipeline fetches recent earnings call transcripts from FMP for universe symbols, runs one Sonnet call per transcript to extract: `guidance_direction` (raised/lowered/maintained/withdrawn/not_given), `tone` (confident/neutral/cautious/defensive), `key_topics`, `forward_outlook`, and `diverges_from_headline` flag. Low-confidence extractions (<0.40) are suppressed. Events written to `macro_events.jsonl` as `event_type=earnings_call_guidance` (TTL 72h). `apex_orchestrator._load_driver_notes()` reads active transcript events and injects them into the Apex prompt under `[EARNINGS CALL INTELLIGENCE]` — Apex uses this as a signal, never as a direct order. `fmp_client.get_earnings_transcript()` added. Pipeline step 7/7 in `run_intelligence_pipeline.py`. Cap: 8 transcripts per run. 22 tests passing.
- **Forces tab Watching section — Active ✅** (2026-05-31, v4.90.0): Forces tab now shows three tiers: Active (price-confirmed), Watching (event-backed, price unconfirmed), Dormant. `CustomerMarketForce` gains `confirmation_status` (active/watching/dormant), `event_summary`, `event_type`. `buildCustomerForces()` accepts `MacroEventContext` — watching forces come from `EVENT_UNCONFIRMED` drivers in the macro event store. `useCustomerBriefing` fetches `/api/intelligence/macro-context` on mount and exposes `watchingForces`. `market_now_builder.py` injects macro event context into `known_conflicts` (unconfirmed drivers surface as divergence warnings) and `what_changed` (confirmed-driver events prepended). Macro risk direction vs regime divergence also surfaced as `known_conflict`. 481 mobile tests passing.
- **Macro Event Layer — Active ✅** (2026-05-31, v4.88.0): News → structured macro intelligence pipeline. `macro_event_layer.py` (INTELLIGENCE layer): extensive keyword pre-filter (100+ terms, 12 categories) gates articles before Sonnet LLM classification. 53 event types, event-type-driven TTL (military_conflict=14d, central_bank_rate_decision=7d, inflation_data=48h). Rate-limited at 30 calls/hr with headline dedup. Stores to `data/intelligence/macro_events.jsonl`. `live_driver_resolver.py` annotates each driver with confirmation status: `CONFIRMED` (event + price agree), `EVENT_UNCONFIRMED` (event implicates driver, price hasn't confirmed), `PRICE_ONLY` (price-only, no event backing), `CONTRADICTED`. Price sensors remain the sole activator when no macro event exists. `intelligence_api.py`: two new routes — `GET /api/intelligence/macro-events` (filterable by hours/driver/domain) and `GET /api/intelligence/macro-context`. `ask-decifer/route.ts` now fetches macro events in parallel; `buildSystemPrompt` gains `MacroEvent[]` param — structured macro event section added to Ask Decifer's system prompt so it can answer questions like "what's backing the oil risk driver." Wired into `news.py` (Yahoo RSS) and `alpaca_news.py` (Benzinga) as fail-soft emits before universe filter. 12 new mobile tests, 480 passing. **Remaining:** droplet needs `git pull` + intelligence-api container restart to pick up new code.
...
- **⭐ TTG CANONICAL THEME SET = 20 themes (the map).** The single source of truth for the customer-facing theme set is `map/lib/chain-definitions.ts` (served at map.decifertrading.com): **20 themes, ~315 unique symbols** — AI Infrastructure (44), Nuclear & Clean Power (20), Semiconductors (20), Cybersecurity (13), GLP-1 & Metabolic Health (23), Healthcare & Biotech (18), Defence (22), Space (10), Quantum Computing (3), Reshoring & Industrial (26), Critical Minerals (13), Precious Metals (14), Oil & Energy (15), Legacy Financials (21), Fintech (12), Real Estate & Housing (25), Consumer (15), Automotive & EV (18), Crypto (13), Water (9). When asked "how many themes" or "what's in a theme", use this. The backend Sprint M12A graph below (10 themes, `data/intelligence/theme_graph/`) is a SEPARATE, smaller dataset that feeds the mobile app — it is NOT the canonical theme count and is a known divergence pending consolidation. Map data is static-imported, so changes need a Vercel redeploy of `decifer-map` to propagate. Full table in `docs/PRODUCT_DEFINITION.md`.
