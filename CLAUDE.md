...
- **Position Research Universe Phase 1 — Shadow observation only ✅** (2026-05-03): Tier D discovery path. Full pipeline instrumentation in place: `tier_d_funnel.jsonl` records stage=pipeline (stages 1-6 attrition), stage=dispatch (Apex classification breakdown), and stage=apex_cap (top-30 hard cap before Apex — whether Tier D is being killed before Apex sees them). Evidence script: `scripts/tier_d_evidence_report.py`. **Phase 2 gate: NOT MET. Section 0b diagnosis (2026-05-31):** 901 scoring cycles analysed. `tier_d_discovery_pool_count = 0` in every cycle — Tier D never generates a pool. PRU has 150 symbols but none flow into scoring. The problem is upstream of the cap — the discovery path itself is broken. Cap is not the issue. Next decision: diagnose why PRU symbols never populate `tier_d_discovery_pool` in `scanner.py`. 30 tests, all passing.
