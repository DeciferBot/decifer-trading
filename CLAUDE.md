...
- **Portfolio Management Engine — Active (HYPOTHETICAL) ✅** (2026-05-22): `rotation_live_v1.py` G1-G9 waterfall retired (never fired in production — root cause: framed PM as "exit one weak position for one blocked buy"). Replaced with deterministic PME: `pm_engine.py`, `pm_thesis.py`, `pm_rails.py`, `pm_observability.py`. 7 action types: HOLD, ADD, DCA, TRIM, FULL_EXIT, ROTATE, DO_NOTHING. Thesis classification (STRENGTHENING/INTACT/PLAYED_OUT/DECAYING/BROKEN/UNKNOWN) drives action generation. **G7 fix**: rail 7 checks `proposed_notional` vs `NLV × PM_MAX_ACTION_NLV_PCT` — NOT full position notional. A 5% NLV position can execute a 1% trim. Safety rails applied post-selection; DO_NOTHING bypasses all rails. Two triggers: `margin_cap_block` (reactive, from orders_core) + `scan_cycle` (proactive, after Track B). Decision log: `data/pm_engine/decisions.jsonl`. `ENABLE_PM_ENGINE=False` — running in HYPOTHETICAL mode. Dashboard tab: "Rotation" → "Portfolio Mgmt", `/api/rotation` tombstoned → `/api/pm`. 17 PM engine tests, all passing. Archived: `rotation_live_v1.py`, `rotation_observability.py`, 4 rotation test files, 2 rotation scripts.
...
- **Test suite**: 2917+ passing (2026-05-22). 17 pre-existing failures (test_quota_policy_promotion, test_signal_dispatch, test_signal_validation_report — all pre-existing, unrelated to PME). PME migration added 17 tests; removed 4 rotation test files. Net test delta: +13.
