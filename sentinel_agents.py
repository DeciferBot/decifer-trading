...
# ══════════════════════════════════════════════════════════════
# PHASE 5 — APEX NEWS_INTERRUPT PAYLOAD BUILDER  (pure function)
# ══════════════════════════════════════════════════════════════
def build_news_trigger_payload(
    ...
    This is a PURE FUNCTION. It performs no LLM call, no dispatch, no order
    submission, and has no side effects other than a single debug log. It
    exists so the Phase 6 cutover can feed the same trigger payload into
    market_intelligence.apex_call() without reshaping data at the call site.

    Inputs:
      trigger             — the dict fired by NewsSentinel.on_trigger
                            (symbol, headlines, keyword_score, direction,
                             urgency, claude_sentiment, claude_confidence,
                             claude_catalyst, sources, ...)
      open_positions      — list of position dicts (or None)
      portfolio_value     — float from dash["portfolio_value"]
      daily_pnl           — float from dash["daily_pnl"]
      regime              — regime dict
      scored_candidate    — optional ScannerPayload for the triggered symbol
                            (Phase 6 will score on-demand and pass it here)
      overnight_research  — optional overnight research string

    Output shape (matches the ApexInput dict consumed by apex_call):
      {
    ...
    trigger_context = {
        ...
        "finbert_sentiment": trigger.get("claude_sentiment"),     # populated by news.claude_sentiment shim today
