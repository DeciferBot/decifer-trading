...
def _make_contract(symbol="AAPL", sec_type="STK", currency="USD", strike=None, right=None, expiry=None):
    c = MagicMock()
    c.symbol = symbol
    c.secType = sec_type
    c.currency = currency
...
class TestPortfolioItemToDict:
    def test_stock_fields(self):
        item = _make_portfolio_item(symbol="TSLA", position=50, market_value=10_000.0, unrealized_pnl=500.0)
    ...
    def test_option_fields(self):
        item = _make_portfolio_item(
            symbol="AAPL", sec_type="OPT", position=2, strike=150.0, right="C", expiry="20250117"
        )
        d = pf._portfolio_item_to_dict(item)
    ...
    def test_negative_position(self):
        item = _make_portfolio_item(position=-100, market_value=-15_000.0, unrealized_pnl=-300.0)
        d = pf._portfolio_item_to_dict(item)
...
class TestPositionKey:
    ...
    def test_option_key_is_composite(self):
        pos = {"symbol": "AAPL", "sec_type": "OPT", "right": "C", "strike": 150.0, "expiry": "20250117"}
...
class TestGetAccountsToAggregate:
    def test_explicit_list_takes_priority(self):
        with patch.dict(pf.CONFIG, {"aggregate_accounts": ["ACC1", "ACC2"], "accounts": {"paper": "PAPER123"}}):
...
class TestGetAggregateSummary:
    ...
    def test_account_error_does_not_crash_summary(self):
        ...
        def portfolio_side_effect(account_id):
            ...
            return [_make_portfolio_item(symbol="AAPL", position=100, market_value=15_000.0, unrealized_pnl=500.0)]
