"""Tests for update_trailing_stops() in orders.py."""
...
def _active_long(symbol="AAPL", entry=100.0, current=100.0, sl=98.5, atr=1.0,
                 sl_order_id=99, hwm=None, status="ACTIVE"):
    return {
        "symbol":          symbol,
        "instrument":      "stock",
        "direction":       "LONG",
...
def _active_short(symbol="TSLA", entry=200.0, current=200.0, sl=202.0, atr=1.0,
                  sl_order_id=88, hwm=None, status="ACTIVE"):
    return {
        "symbol":          symbol,
        "instrument":      "stock",
        "direction":       "SHORT",
...
class TestTrailingStopLong:
    ...
    def test_raises_stop_on_new_high(self):
        ...
        orders.active_trades = {"AAPL": trade}
        ...
        assert orders.active_trades["AAPL"]["sl"] == 99.5
        assert orders.active_trades["AAPL"]["high_water_mark"] == 101.5
    ...
    def test_does_not_lower_stop(self):
        ...
        orders.active_trades = {"AAPL": trade2}
        ib2 = _make_ib()
        with patch("orders.get_contract", return_value=MagicMock()), \
             patch("orders.CONFIG", {
                 "trailing_stop_enabled": True,
    ...
    def test_no_update_when_price_unchanged(self):
        ...
        assert orders.active_trades["AAPL"]["sl"] == 98.5
...
class TestTrailingStopShort:
    ...
    def test_lowers_stop_on_new_low(self):
        ...
        orders.active_trades = {"TSLA": trade}
        ...
        assert orders.active_trades["TSLA"]["sl"] == 200.0
        assert orders.active_trades["TSLA"]["high_water_mark"] == 198.0
        assert ib.placeOrder.called
...
class TestTrailingStopSkips:
    ...
    def test_skips_options(self):
        ...
        ib = _make_ib()
        with patch("orders.CONFIG", {"trailing_stop_enabled": True, "atr_trail_multiplier": 2.0}):
            orders.update_trailing_stops(ib)
    ...
    def test_skips_when_no_sl_order_id(self):
        ...
        ib = _make_ib()
        with patch("orders.get_contract", return_value=MagicMock()), \
             patch("orders.CONFIG", {"trailing_stop_enabled": True, "atr_trail_multiplier": 2.0}):
            orders.update_trailing_stops(ib)
    ...
    def test_skips_when_disabled(self):
        ...
        ib = _make_ib()
        with patch("orders.CONFIG", {"trailing_stop_enabled": False, "atr_trail_multiplier": 2.0}):
            orders.update_trailing_stops(ib)
