...
def _make_snapshot(delta: float, iv: float, ctype: str, strike: float = 100.0, expiry_days: int = 30):
...
class TestGetIvSkewClientGating:
    ...
    def test_returns_none_when_alpaca_not_installed(self):
        ...
        iv_skew._client = None
        with mock.patch.dict("sys.modules", {"alpaca.data.historical.option": None, "alpaca.data.requests": None}):
...
class TestGetIvSkewChainParsing:
    ...
    def test_selects_correct_atm_call_and_otm_put(self):
        ...
        with (
            self._patch_client(chain),
            mock.patch.dict(
                "sys.modules", {"alpaca.data.requests": types.SimpleNamespace(OptionChainRequest=mock.MagicMock())}
            ),
        ):
            result = iv_skew.get_iv_skew("AAPL")
...
class TestGetIvSkewResultStructure:
    ...
    def test_result_has_all_required_keys(self):
        ...
        if result is not None:
            for key in ["skew", "otm_put_iv", "atm_call_iv", "iv_skew_score", "iv_skew_dir", "expiry", "source"]:
