...
for _mod_name in [
    ...
    "social_sentiment", "ml_engine", "portfolio_optimizer",
]:
    ...
    _stub.update_equity_high_water_mark = MagicMock()
...
class TestSubscriptionRegistry(unittest.TestCase):
    ...
    def test_register_stores_entry(self):
        ...
        self.assertIn("AAPL", bot._subscription_registry)

    def test_register_stores_correct_params(self):
        params = {"type": "ticker", "exchange": "SMART"}
        bot._register_subscription("TSLA", params)
        self.assertEqual(bot._subscription_registry["TSLA"], params)

    def test_register_overwrites_existing_key(self):
        bot._register_subscription("MSFT", {"type": "ticker"})
        bot._register_subscription("MSFT", {"type": "pnl"})
        self.assertEqual(bot._subscription_registry["MSFT"]["type"], "pnl")

    def test_unregister_removes_entry(self):
        bot._register_subscription("GOOG", {"type": "ticker"})
        bot._unregister_subscription("GOOG")
        self.assertNotIn("GOOG", bot._subscription_registry)

    def test_unregister_nonexistent_is_noop(self):
        """Should not raise KeyError."""
        bot._unregister_subscription("DOES_NOT_EXIST")

    def test_registry_starts_empty_after_reset(self):
        bot._register_subscription("X", {"type": "ticker"})
        _reset_reconnect_state()
        self.assertEqual(len(bot._subscription_registry), 0)

    def test_restore_subscriptions_calls_reqPnL(self):
        """_restore_subscriptions calls ib.reqPnL for PnL subscriptions."""
        bot._register_subscription("__pnl__", {"type": "pnl", "account": "DU999"})
        mock_ib = MagicMock()
    ...
    def test_restore_subscriptions_calls_reqMktData(self):
        ...
        bot._register_subscription("NVDA", {"type": "ticker"})
        mock_ib = MagicMock()
        with patch.object(bot, "ib", mock_ib):
            bot._restore_subscriptions()
    ...
    def test_restore_subscriptions_multiple(self):
        ...
        bot._register_subscription("__pnl__", {"type": "pnl", "account": "DU1"})
        bot._register_subscription("AMD", {"type": "ticker"})
        bot._register_subscription("SPY", {"type": "ticker"})
        mock_ib = MagicMock()
        with patch.object(bot, "ib", mock_ib):
            bot._restore_subscriptions()
    ...
    def test_restore_subscriptions_unknown_type_no_raise(self):
        ...
        bot._register_subscription("WEIRD", {"type": "unknown_type"})
        mock_ib = MagicMock()
        with patch.object(bot, "ib", mock_ib):
    ...
    def test_restore_subscriptions_exception_in_one_does_not_abort(self):
        ...
        bot._register_subscription("__pnl__", {"type": "pnl", "account": "DU1"})
        bot._register_subscription("FAIL", {"type": "ticker"})
        bot._register_subscription("OK",   {"type": "ticker"})
...
class TestOnDisconnected(unittest.TestCase):
    ...
    def test_second_call_while_reconnecting_ignored(self):
        started = []
        with patch("threading.Thread", side_effect=self._make_fake_thread(started)):
            bot._on_disconnected()   # first call
            bot._on_disconnected()   # second call — must be ignored
        self.assertEqual(len(started), 1,
                         "Only one reconnect thread should start")
...
class TestReconnectWorkerBackoff(unittest.TestCase):
    ...
    def test_backoff_capped_at_max_wait(self):
        ...
        sleep_calls = self._run_worker_all_fail()
        self.assertTrue(all(s <= 4 for s in sleep_calls),
                        f"Sleep exceeded max: {sleep_calls}")
    ...
    def test_stops_after_success(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            with patch.object(bot, "_restore_subscriptions"):
                with patch("time.sleep"):
                    bot._reconnect_worker()
...
class TestReconnectWorkerState(unittest.TestCase):
    ...
    def test_restore_subscriptions_called_on_success(self):
        ...
        mock_ib.connect = MagicMock()
        with patch.object(bot, "ib", mock_ib):
            with patch.object(bot, "_restore_subscriptions") as mock_restore:
                with patch("time.sleep"):
                    bot._reconnect_worker()
    ...
    def test_exhaustion_triggers_alert_with_correct_count(self):
        ...
        mock_ib.connect = MagicMock(side_effect=OSError("refused"))
        with patch.object(bot, "ib", mock_ib):
            with patch.object(bot, "_send_reconnect_exhausted_alert") as mock_alert:
                with patch("time.sleep"):
                    bot._reconnect_worker()
    ...
    def test_connect_called_with_correct_args(self):
        ...
        mock_ib.connect.assert_called_once_with(
            "192.168.1.50", 4001, clientId=7, readonly=False
        )
...
class TestHeartbeatWorker(unittest.TestCase):
    ...
    def test_heartbeat_exception_does_not_crash(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            with patch("time.sleep", side_effect=fake_sleep):
                try:
                    bot._heartbeat_worker()  # must not propagate RuntimeError
                except StopIteration:
                    pass
...
class TestConnectIbkr(unittest.TestCase):
    ...
    def test_returns_true_on_success(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            with patch("threading.Thread", side_effect=factory):
    ...
    def test_registers_disconnect_handler(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            with patch("threading.Thread", side_effect=factory):
                bot.connect_ibkr()
        self.assertIn(bot._on_disconnected, mock_ib.disconnectedEvent)
    ...
    def test_handler_not_registered_on_failure(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            bot.connect_ibkr()
        self.assertNotIn(bot._on_disconnected, mock_ib.disconnectedEvent)
    ...
    def test_pnl_subscription_registered(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            with patch("threading.Thread", side_effect=factory):
                bot.connect_ibkr()
        self.assertIn("__pnl__", bot._subscription_registry)
        self.assertEqual(bot._subscription_registry["__pnl__"]["type"], "pnl")
    ...
    def test_pnl_subscription_not_registered_on_failure(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            bot.connect_ibkr()
        self.assertNotIn("__pnl__", bot._subscription_registry)
    ...
    def test_handler_registered_exactly_once_on_double_call(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            with patch("threading.Thread", side_effect=factory):
                bot.connect_ibkr()
                bot.connect_ibkr()
        count = mock_ib.disconnectedEvent.count(bot._on_disconnected)
    ...
    def test_ib_connect_called_with_correct_args(self):
        ...
        with patch.object(bot, "ib", mock_ib):
            with patch("threading.Thread", side_effect=factory):
                bot.connect_ibkr()
        mock_ib.connect.assert_called_once_with(
            "10.0.0.1", 4001, clientId=5, readonly=False
        )
...
class TestReconnectExhaustedAlert(unittest.TestCase):
    ...
    def test_no_webhook_does_not_raise(self):
        ...
        bot._send_reconnect_exhausted_alert(10)  # must not raise

    def test_dashboard_status_updated(self):
        bot.CONFIG["reconnect_alert_webhook"] = ""
        bot._send_reconnect_exhausted_alert(5)
    ...
    def test_webhook_post_attempted(self):
        ...
        with patch("urllib.request.urlopen") as mock_urlopen:
            ...
            bot._send_reconnect_exhausted_alert(10)
    ...
    def test_webhook_payload_contains_attempt_count(self):
        ...
        with patch("urllib.request.urlopen", side_effect=capture):
            bot._send_reconnect_exhausted_alert(7)
...
@pytest.mark.parametrize("base,max_w,attempts,expected", [
    (1,  16, 5, [1, 2, 4, 8, 16]),
...
class TestModuleLevelState(unittest.TestCase):
    ...
    def test_reconnect_lock_is_lock(self):
        self.assertIsInstance(bot._reconnect_lock, type(threading.Lock()))
    ...
    def test_subscription_registry_is_dict(self):
        self.assertIsInstance(bot._subscription_registry, dict)
