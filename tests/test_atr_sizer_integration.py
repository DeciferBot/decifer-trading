...
import risk                    # noqa: E402 — use the real module
import orders                  # noqa: E402 — use the real module
...
from orders import execute_buy             as _real_execute_buy             # noqa: E402
from risk   import calculate_position_size as _real_calculate_position_size # noqa: E402
from risk   import calculate_stops         as _real_calculate_stops         # noqa: E402
from risk   import get_session             as _real_get_session             # noqa: E402
from risk   import get_vix_rank            as _real_get_vix_rank            # noqa: E402
...
def _default_plan() -> "ExecutionPlan":
...
class TestAtrForwardedToSizer:
    ...
    def test_atr_kwarg_forwarded_to_calculate_position_size(self):
        ...
        assert "atr" in kw, (
            "calculate_position_size was not called with the atr keyword — "
            "ATR cap is silently disabled"
        )
        assert kw["atr"] == pytest.approx(_ATR_TIGHT), (
            f"Expected atr={_ATR_TIGHT}, got atr={kw['atr']!r}"
        )
    ...
    def test_zero_atr_forwarded_not_replaced_by_default(self):
        ...
        kw = spy.call_args.kwargs
        assert kw.get("atr", -1) == pytest.approx(0.0), (
            "atr=0 must not be silently replaced before position sizing"
        )
    ...
    def test_atr_also_forwarded_to_calculate_stops(self):
        ...
        assert args[1] == pytest.approx(_ATR_TIGHT), (
            f"calculate_stops received atr={args[1]!r}, expected {_ATR_TIGHT}"
        )
...
class TestExecutionAgentOrderTypeRouting:
    ...
    def _plan(self, order_type: str) -> "ExecutionPlan":
        from execution_agent import ExecutionPlan
        return ExecutionPlan(
    ...
    def test_mkt_plan_uses_market_order(self):
        ...
        assert mkt_mock.called, (
            "execution_agent returned order_type='MKT' but MarketOrder was never constructed"
        )
    ...
    def test_limit_plan_does_not_use_market_order(self):
        ...
        assert not mkt_mock.called, (
            "execution_agent returned order_type='LIMIT' but MarketOrder was constructed"
        )
        assert lmt_mock.called, (
            "execution_agent returned order_type='LIMIT' but LimitOrder was never constructed"
        )
    ...
    def test_exec_plan_params_passed_to_fill_watcher(self):
        ...
        try:
            with patch.dict(sys.modules, {"fill_watcher": fw_module}):
                _real_execute_buy(
                    ib=ib, symbol="AMD",
                    price=_PRICE, atr=_ATR_TIGHT, score=_SCORE,
                    portfolio_value=_PORTFOLIO, regime=_REGIME_BULL,
                    tranche_mode=False,
                )
        finally:
            for cm in reversed(ctx_mgrs):
        ...
        assert fw_mock.called, (
            "FillWatcher was never constructed — fill_watcher_params cannot be verified"
        )
        fw_kwargs = fw_mock.call_args.kwargs
        assert "watcher_params" in fw_kwargs, (
            "FillWatcher constructor was not passed watcher_params kwarg"
        )
...
class TestAtrCapEndToEnd:
    ...
    def test_tight_atr_produces_fewer_shares_than_loose_atr(self):
        ...
        with patch.object(risk, "get_vix_rank", return_value=0.0), \
            ...
            qty_tight = _real_calculate_position_size(
                _PORTFOLIO, _PRICE, _SCORE, _REGIME_BULL, atr=_ATR_TIGHT
            )
            qty_loose = _real_calculate_position_size(
                _PORTFOLIO, _PRICE, _SCORE, _REGIME_BULL, atr=_ATR_LOOSE
            )
    ...
    def test_atr_cap_disabled_when_atr_is_zero(self):
        ...
        with patch.object(risk, "get_vix_rank", return_value=0.0), \
             patch.object(risk, "get_session",  return_value="MARKET"):
            qty_zero = _real_calculate_position_size(
                _PORTFOLIO, _PRICE, _SCORE, _REGIME_BULL, atr=0.0
            )
            qty_none = _real_calculate_position_size(
                _PORTFOLIO, _PRICE, _SCORE, _REGIME_BULL
            )

        assert qty_zero == qty_none, (
            f"atr=0 and atr omitted must produce identical sizing; "
            f"got zero_atr={qty_zero}, no_atr={qty_none}"
        )
