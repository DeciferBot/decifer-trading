...
def run_scan():
    ...
    try:
        ...
        # Attach apex_cap_score before sorting. Non-Tier-D candidates receive
        # their raw signal score unchanged. Tier D candidates with score >= 18
        # receive a bounded research bonus (max +8) so high-conviction names
        ...
        if len(_cut_candidates_deduped) > _CAP_LIMIT:
            ...
            for _rc in _td_recovered_log[:5]:
                clog("INFO",
                     f"  RECOVERED_TD: {_rc.get('symbol')} score={_rc.get('score')} "
                     f"apex_cap_score={_rc.get('apex_cap_score', 0):.2f} "
                     f"discovery={_rc.get('discovery_score')} adj_discovery={_rc.get('adjusted_discovery_score')} "
                     f"archetype={_rc.get('primary_archetype')} bucket={_rc.get('universe_bucket')} "
                     f"risk_penalty={_rc.get('risk_penalty_pts')}")
            for _dc in _non_td_displ_log[:5]:
                clog("INFO",
        ...
        try:
            ...
            _live_compare_record = {
                ...
                "non_tier_d_displaced_symbols":     [c.get("symbol") for c in _non_td_displ_cmp],
                "top_tier_d_recovered": [
                    {
                        "symbol":                   c.get("symbol"),
                        "score":                    c.get("score"),
                        "apex_cap_score":           c.get("apex_cap_score"),
                        ...
                        "primary_archetype":        c.get("primary_archetype"),
                        "universe_bucket":          c.get("universe_bucket"),
                        "risk_penalty_pts":         c.get("risk_penalty_pts"),
                    }
                    for c in _td_recovered_cmp[:10]
                ],
                "top_non_tier_d_displaced": [
                    {
                        "symbol":         c.get("symbol"),
                        "score":          c.get("score"),
                        "apex_cap_score": c.get("apex_cap_score", c.get("score")),
                    }
                    for c in _non_td_displ_cmp[:10]
                ],
                "old_raw_cap_tier_d_selected_count":     sum(1 for c in _old_sorted_cmp[:_CAP_LIMIT] if c.get("scanner_tier") == "D"),
                "old_raw_cap_non_tier_d_selected_count": sum(1 for c in _old_sorted_cmp[:_CAP_LIMIT] if c.get("scanner_tier") != "D"),
            }
            with open(_funnel_path, "a") as _lcf:
