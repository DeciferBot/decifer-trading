...
def dispatch_forced_exit(
    ...
    *,
    ...
    # RC-1: per-symbol lock prevents two threads from executing a forced exit
    # concurrently for the same symbol. Without this, two calls can both reset
    # EXITING → ACTIVE and both call execute_sell, submitting two SELL orders
    # that both fill — inverting the position into a net short (the MS incident).
    from orders_state import _get_symbol_lock as _gsl
    with _gsl(symbol):
