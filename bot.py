...
# ═══════════════════════════════════════════════════════════════
# CATALYST SENTINEL — real-time M&A / acquisition trigger handler
# ═══════════════════════════════════════════════════════════════
_catalyst_sentinel: CatalystSentinel = None
_catalyst_trades_today = 0
_catalyst_trade_date: str = ""
...
def handle_catalyst_trigger(trigger: dict):
    ...
    This runs on the catalyst sentinel's background thread.
    """
    global _catalyst_trades_today, _catalyst_trade_date
    ...
    if _catalyst_trade_date != today:
        _catalyst_trades_today = 0
        _catalyst_trade_date = today

    max_per_day = CONFIG.get("catalyst_max_trades_per_day", 2)
    if _catalyst_trades_today >= max_per_day:
        clog("RISK", f"Catalyst rate limit: {_catalyst_trades_today}/{max_per_day} trades today — skipping {sym}")
        return
    ...
    # ── EDGAR signals without a resolved ticker are logged only ─
    if not sym or sym == "?":
    ...
    try:
        ...
        if action == "BUY":
            # Apply catalyst position-size multiplier on top of sentinel multiplier
...
def _execute_catalyst_buy(decision: dict, portfolio_value: float,
                          regime: dict, trigger: dict):
    """
    Execute a catalyst-triggered buy order.
    Mirrors _execute_sentinel_buy with reduced sizing and a [CATALYST] tag.
    """
    sym       = decision.get("symbol", "")
    qty       = decision.get("qty", 0)
    sl        = decision.get("sl", 0)
...
""")
    ...
    global _catalyst_sentinel
    if CONFIG.get("catalyst_sentinel_enabled", True):
        _catalyst_sentinel = CatalystSentinel(
            get_universe_fn=_get_sentinel_universe,
            on_trigger_fn=handle_catalyst_trigger,
            ib=ib,
        )
        _catalyst_sentinel.start()
        dash["catalyst_triggers"] = []
        dash["catalyst_sentinel_stats"] = _catalyst_sentinel.stats
        clog("INFO",
