...
class MarketObservation:
    ...
    def to_prompt_text(self) -> str:
        ...
        for sym, s in self.assets.items():
            ...
            lines.append(
                f"  {sym:<5} {s.label:<32} ${s.price:>8.2f}  {s.change_1d:>+6.2f}%  {s.change_5d:>+6.2f}%  {ma}"
            )
...
_cached_obs: MarketObservation | None = None
...
def _cache_valid() -> bool:
    ...
    return (datetime.now(UTC) - _cache_time) < ttl
...
def _compute_correlations(bars: dict[str, pd.DataFrame]) -> dict[str, dict[str, float]]:
    ...
    for s1 in corr_matrix.index:
        result[str(s1)] = {str(s2): round(float(v), 3) for s2, v in corr_matrix[s1].items()}
    return result
...
def _compute_sector_vs_spy(
    ...
    spy_5d = assets.get("SPY")
    if spy_5d is None:
        return {}
    spy_ret = spy_5d.change_5d
    return {sym: round(assets[sym].change_5d - spy_ret, 3) for sym in SECTOR_ETFS if sym in assets}
...
def get_market_observation(force_refresh: bool = False) -> MarketObservation:
    ...
    log.info(f"market_observer: {len(assets)} assets, VIX={vix:.1f}, {len(errors)} fetch errors")
    return obs
